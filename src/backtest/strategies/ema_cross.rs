...
#[derive(Deserialize)]
struct Params { fast: usize, slow: usize, qty: f64 }
...
impl EmaCross {
    pub fn from_params(v: &serde_json::Value) -> Result<Self> {
        ...
        Ok(Self {
            fast_n: p.fast, slow_n: p.slow,
            qty: Decimal::from_f64_retain(p.qty).unwrap_or_default(),
            fast_ema: None, slow_ema: None, prev_diff: None, bars_seen: 0,
        })
    }
}
...
impl Strategy for EmaCross {
    fn on_bar(&mut self, bar: &Bar, ctx: &mut Context) {
        ...
        self.bars_seen += 1;
        if self.bars_seen < self.slow_n { return; }
